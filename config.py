"""
⚙️ CẤU HÌNH BOT v7.5 - REAL BROWSER MODE (FIXED)
Camoufox tự launch Firefox thật (headless=False).
Không cần Edge/Chrome mở sẵn, không cần CDP port.

FIXES v7.5:
  ✅ MAX_CONCURRENT_PROCESSING thêm vào (thiếu → bot dùng default 24)
  ✅ MESSAGE_QUEUE_MAXSIZE tăng 500 → 2000 (đủ buffer 15 kênh burst)
  ✅ MESSAGE_WORKERS tăng 3 → 4 (đủ dispatch tốc độ cao)
  ✅ SITE_CODE_DEDUP_TTL tăng 15 → 30s (tránh submit trùng khi burst)
  ✅ MIN_DELAY_BETWEEN_SUBMITS giảm 0.8 → 0.5s (nhanh hơn)
  ✅ Bỏ MAX_TELEGRAM_DELAY_SECONDS (không dùng nữa)
  ✅ AUTO_OPEN_MISSING_TABS thêm vào (cần cho TabPool on-demand)
  ✅ Thêm TIMEZONE rõ ràng (Asia/Ho_Chi_Minh)
...
class Config:
    ...
    # ==========================================
    # 🔄 POLLING FALLBACK
    # Bật để bot chủ động hỏi tin mới từ các kênh mỗi N giây,
    # thay vì chỉ chờ Telegram push (push đôi khi bị delay/mất).
    # ==========================================
    POLLING_ENABLED          = get_bool_env("POLLING_ENABLED", True)
    POLLING_INTERVAL_SECONDS = get_int_env("POLLING_INTERVAL_SECONDS", 15)

    # ==========================================
    # 🕐 TIMEZONE (quan trọng — dùng cho log & daily reset)
    # ==========================================
    # Máy chạy ở VN → đặt TZ=Asia/Ho_Chi_Minh để datetime.now() = giờ VN
    # BOT_START_TIME trong main_script dùng UTC để so sánh với Telegram msg_date
    TIMEZONE = "Asia/Ho_Chi_Minh"  # chỉ dùng cho display, không ảnh hưởng so sánh UTC
    ...
    MAX_LOG_SIZE = get_int_env("MAX_LOG_SIZE", 10485760)  # 10MB
    ...
    CODE_FILTER_GROUPS = {
        ...
        "qq88": {
            ...
            "min_entropy": 2.0,
            "uppercase_min_entropy": 2.5,
            "soft_blacklist": ["GAME", "VIP", "NAP", "RUT", "LINK"],
        },
        "uy88": {
        ...
        "mmoo": {
            ...
            "min_special_chars": 0,
            "min_entropy": 1.5,
            "uppercase_min_entropy": 2.0,
            "soft_blacklist": ["CODE", "GAME", "FREE", "VIP", "NAP", "RUT"],
            "enable_placeholder_mode": True,
        },
        "default": {
    ...
    # 🖥️ ACCOUNT GROUPS
    # ==========================================
    # v7.5: Không còn dùng CDP/Edge. Dict này chỉ nhóm tài khoản để tham chiếu.
    CDP_CONNECTIONS = {
        "main": [
            "kaoboy012",
            "kuuteo012",
    ...
    # ==========================================
    # 🖥️ REAL BROWSER (Camoufox)
    # ==========================================
    BROWSER_PROFILE_BASE_DIR = os.getenv("BROWSER_PROFILE_BASE_DIR", "browser_profiles")
    BROWSER_SLOW_MO          = get_int_env("BROWSER_SLOW_MO", 100)
    ...
    # ✅ FIX: Cần thiết cho TabPool on-demand navigation
    ...
    # Telegram admin nhận cảnh báo CF (0 = tắt)
    ...
    # ✅ FIX: Tăng 15 → 30s để tránh submit trùng khi nhiều kênh burst cùng lúc
    ...
    # ✅ FIX: Giảm 0.8 → 0.5s để submit nhanh hơn
    ...
    # ==========================================
    # ✅ FIX: Tăng queue 500 → 2000 (đủ cho 15 kênh burst cùng lúc)
    MESSAGE_QUEUE_MAXSIZE = get_int_env("MESSAGE_QUEUE_MAXSIZE", 2000)

    # ✅ FIX: Tăng workers 3 → 4
    MESSAGE_WORKERS = get_int_env("MESSAGE_WORKERS", 4)

    # ✅ FIX: Thêm MAX_CONCURRENT_PROCESSING (thiếu trong config cũ → main dùng default 24)
