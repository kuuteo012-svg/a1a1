...
def print_version_info():
    ...
    try:
        ...
        try:
            ...
            camoufox_version = pkg_version("camoufox")
        except Exception:
            try:
                import camoufox
                camoufox_version = getattr(camoufox, "__version__", "installed")
            except Exception:
                camoufox_version = "unknown"

        logger.info(
            f"📦 Bot v{BOT_VERSION} ({BOT_BUILD_DATE}) | "
            f"Telethon {telethon.__version__} | Camoufox {camoufox_version}"
        )
...
class ConfigValidator:
    ...
    def validate_all():
        ...
        # 1. Check Telegram API credentials
        if not getattr(Config, "API_ID", 0) or Config.API_ID == 0:
            errors.append("❌ API_ID chưa được set trong .env (bắt buộc)")
        api_hash = getattr(Config, "API_HASH", "") or ""
        if not api_hash or len(api_hash.strip()) < 10:
            errors.append("❌ API_HASH chưa được set trong .env (bắt buộc)")

        # 2. Check session
        if not Config.SESSION_NAME:
            errors.append("❌ SESSION_NAME rỗng — chưa set trong .env")
        ...
        if Config.CODE_MAX_LENGTH > 50:
            warnings.append("⚠️ CODE_MAX_LENGTH qua lon (khuyen nghi <= 50)")
        ...
        try:
            ...
            def _win_handler(ctrl_type):
                if ctrl_type in (CTRL_C_EVENT, CTRL_BREAK_EVENT,
                    ...
                    self._do_shutdown(bot_state, reason="Windows console event")
...
class DatabaseBackup:
    ...
    def backup(self, db_path: str) -> bool:
        ...
        try:
            ...
            # ✅ FIX: Dùng SQLite backup API thay shutil.copy2
            # shutil.copy2 trên WAL-mode database có thể tạo backup corrupt
            # nếu đang có ghi đồng thời. SQLite backup API an toàn 100%.
            ...
            try:
                ...
                src_conn.close()
                dst_conn.close()
            logger.info(f"✅ Backup database (safe): {backup_path}")
